...
class RunGroup {
  ...
  public long computeWrapSize(ConstraintWidgetContainer paramConstraintWidgetContainer, int paramInt) {
    ...
    if (bool1 && bool2) {
      ...
      l3 = this.firstRun.start.margin + l5 + l2 + l3;
      paramInt = this.firstRun.end.margin;
    } else {
      if (bool1) {
        l3 = Math.max(traverseStart(this.firstRun.start, this.firstRun.start.margin), this.firstRun.start.margin + l2);
      } else if (bool2) {
        long l = traverseEnd(this.firstRun.end, this.firstRun.end.margin);
        l3 = -this.firstRun.end.margin;
        l3 = Math.max(-l, l3 + l2);
      } else {
        l3 = this.firstRun.start.margin + this.firstRun.getWrapDimension();
        paramInt = this.firstRun.end.margin;
        l3 -= paramInt;
      } 
      return l3;
...
/* Location:              /home/austin/Documents/CTFFolder/CTF-Writeups/Razi-CTF2020/Android/chasing_a_lock/chasing a lock/classes-dex2jar.jar!/androidx/constraintlayout/solver/widgets/analyzer/RunGroup.class
