...
public abstract class WidgetRun implements Dependency {
  ...
  protected final int getLimitedDimension(int paramInt1, int paramInt2) {
    if (paramInt2 == 0) {
      paramInt2 = this.widget.mMatchConstraintMaxWidth;
      int i = Math.max(this.widget.mMatchConstraintMinWidth, paramInt1);
      if (paramInt2 > 0)
      ...
      paramInt2 = this.widget.mMatchConstraintMaxHeight;
      int i = Math.max(this.widget.mMatchConstraintMinHeight, paramInt1);
      if (paramInt2 > 0)
        i = Math.min(paramInt2, paramInt1); 
      paramInt2 = paramInt1;
      if (i != paramInt1)
  ...
  protected void updateRunCenter(Dependency paramDependency, ConstraintAnchor paramConstraintAnchor1, ConstraintAnchor paramConstraintAnchor2, int paramInt) {
    ...
    if (((DependencyNode)paramDependency).resolved && dependencyNode.resolved) {
      float f;
      int i = ((DependencyNode)paramDependency).value + paramConstraintAnchor1.getMargin();
      int j = dependencyNode.value - paramConstraintAnchor2.getMargin();
      int k = j - i;
      ...
      this.start.resolve((int)(paramInt + 0.5F + (j - paramInt - i) * f));
  ...
  enum RunType {
    ...
    static {
      CENTER = new RunType("CENTER", 3);
      $VALUES = new RunType[] { NONE, START, END, CENTER };
    }
  }
}


/* Location:              /home/austin/Documents/CTFFolder/CTF-Writeups/Razi-CTF2020/Android/chasing_a_lock/chasing a lock/classes-dex2jar.jar!/androidx/constraintlayout/solver/widgets/analyzer/WidgetRun.class
