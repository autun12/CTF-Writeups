...
class TouchResponse {
  ...
  static {
    ...
    float[] arrayOfFloat6 = { 1.0F, 0.5F };
    TOUCH_SIDES = new float[][] { { 0.5F, 0.0F }, arrayOfFloat1, arrayOfFloat2, arrayOfFloat3, arrayOfFloat4, arrayOfFloat5, arrayOfFloat6 };
    ...
    TOUCH_DIRECTION = new float[][] { arrayOfFloat1, { 0.0F, 1.0F }, arrayOfFloat2, arrayOfFloat3, { -1.0F, 0.0F }, arrayOfFloat4 };
  }
  ...
  private void fill(TypedArray paramTypedArray) {
    ...
    for (byte b = 0; b < i; b++) {
      ...
      if (j == R.styleable.OnSwipe_touchAnchorId) {
        ...
        this.mTouchAnchorSide = paramTypedArray.getInt(j, this.mTouchAnchorSide);
        float[][] arrayOfFloat = TOUCH_SIDES;
        j = this.mTouchAnchorSide;
        this.mTouchAnchorX = arrayOfFloat[j][0];
        this.mTouchAnchorY = arrayOfFloat[j][1];
      } else if (j == R.styleable.OnSwipe_dragDirection) {
        this.mTouchSide = paramTypedArray.getInt(j, this.mTouchSide);
  ...
  void processTouchEvent(MotionEvent paramMotionEvent, MotionLayout.MotionTracker paramMotionTracker, int paramInt, MotionScene paramMotionScene) {
    ...
    if (paramInt != 0) {
      if (paramInt != 1) {
        ...
        paramMotionTracker.computeCurrentVelocity(1000);
        float f3 = paramMotionTracker.getXVelocity();
        ...
        if (f3 != 0.0F && f3 != 1.0F) {
          ...
          if (paramInt != 3) {
            ...
            motionLayout.touchAnimateTo(paramInt, f3, f1);
            if (0.0F >= f2 || 1.0F <= f2)
  ...
  void scrollUp(float paramFloat1, float paramFloat2) {
    ...
    if (f2 != 0.0F) {
      ...
      paramFloat1 = paramFloat2 * f3 / arrayOfFloat[1];
    } 
    paramFloat2 = f1;
    ...
    if (paramFloat2 != 0.0F) {
      ...
      if ((i & j) != 0) {
        ...
        if (paramFloat2 < 0.5D)
          f1 = 0.0F; 
        motionLayout.touchAnimateTo(j, f1, paramFloat1);
      } 
    } 
  }
...
/* Location:              /home/austin/Documents/CTFFolder/CTF-Writeups/Razi-CTF2020/Android/chasing_a_lock/chasing a lock/classes-dex2jar.jar!/androidx/constraintlayout/motion/widget/TouchResponse.class
