...
public abstract class SplineSet {
  ...
  public void setup(int paramInt) {
    ...
    byte b = 1;
    for (i = 1; b < this.count; i = j) {
      int[] arrayOfInt = this.mTimePoints;
      int j = i;
      if (arrayOfInt[b - 1] != arrayOfInt[b])
        j = i + 1; 
      b++;
    } 
  ...
  static class CustomSet extends SplineSet {
    ...
    public void setup(int param1Int) {
      ...
      double[] arrayOfDouble = new double[i];
      this.mTempValues = new float[j];
      double[][] arrayOfDouble1 = new double[i][j];
      j = 0;
      while (j < i) {
        int k = this.mConstraintAttributeList.keyAt(j);
        ConstraintAttribute constraintAttribute = (ConstraintAttribute)this.mConstraintAttributeList.valueAt(j);
...
/* Location:              /home/austin/Documents/CTFFolder/CTF-Writeups/Razi-CTF2020/Android/chasing_a_lock/chasing a lock/classes-dex2jar.jar!/androidx/constraintlayout/motion/widget/SplineSet.class
