  ...
  static class CustomSet extends TimeCycleSplineSet {
    ...
    public boolean setProperty(View param1View, float param1Float, long param1Long, KeyCache param1KeyCache) {
      ...
      for (byte b = 0; b < this.mCache.length; b++) {
        boolean bool2;
        boolean bool1 = this.mContinue;
        if (this.mTempValues[b] != 0.0D) {
          bool2 = true;
        } else {
          bool2 = false;
        } 
        this.mContinue = bool1 | bool2;
    ...
    public void setup(int param1Int) {
      ...
      int j = ((ConstraintAttribute)this.mConstraintAttributeList.valueAt(0)).noOfInterpValues();
      double[] arrayOfDouble = new double[i];
      int k = j + 2;
      this.mTempValues = new float[k];
      this.mCache = new float[j];
      double[][] arrayOfDouble1 = new double[i][k];
      k = 0;
      while (k < i) {
        int m = this.mConstraintAttributeList.keyAt(k);
        ConstraintAttribute constraintAttribute = (ConstraintAttribute)this.mConstraintAttributeList.valueAt(k);
        float[] arrayOfFloat = (float[])this.mWaveProperties.valueAt(k);
...
/* Location:              /home/austin/Documents/CTFFolder/CTF-Writeups/Razi-CTF2020/Android/chasing_a_lock/chasing a lock/classes-dex2jar.jar!/androidx/constraintlayout/motion/widget/TimeCycleSplineSet.class
